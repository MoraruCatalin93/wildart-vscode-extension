...
function getAvailableModels(): Promise<string[]> {
...
function getProjectContext(): string {
    ...
    function walk(dir: string, level = 0): string {
        if (level > 2) return '';
        let result = '';
        try {
...
function runGitCommand(command: string, cwd: string): Promise<string> {
    return new Promise((resolve) => {
        exec(`git ${command}`, { cwd }, (err, stdout, stderr) => {
            resolve(stdout || stderr || err?.message || '');
        });
    });
}

async function getGitContext(root: string): Promise<string> {
    try {
...
function readMemory(memPath: string): string {
    try {
...
function writeMemory(memPath: string, content: string): void {
    try { fs.writeFileSync(memPath, content, 'utf8'); } catch {}
}

function buildSystemPrompt(language: 'ro' | 'en', projectCtx: string): string {
    if (language === 'en') {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('wildartAgent', provider)
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('wildartAgent.clearChat', () => provider.clearChat())
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('wildartAgent.gitCommit', () => provider.quickGitCommit())
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('wildartAgent.selectModel', () => provider.selectModel())
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('wildartAgent.selectLanguage', () => provider.selectLanguage())
    );
}

// ─── Panel ──────────────────────────────────────────────────────────────────

class WildArtAgentPanel implements vscode.WebviewViewProvider {
    ...
    constructor(private readonly _context: vscode.ExtensionContext) {
        ...
        const saved = _context.globalState.get<AgentSettings>('wildartSettings');
        if (saved) this._settings = { ...this._settings, ...saved };
    }
    ...
    async selectLanguage() {
        ...
        const selected = await vscode.window.showQuickPick(
            ...
            { title: 'Selectează limba agentului' }
        );
        if (selected) {
    ...
    async quickGitCommit() {
        ...
        if (!root) { vscode.window.showErrorMessage('Niciun proiect deschis'); return; }
        ...
        const commitMsg = await askAgent(
            `${prompt}\n\n${diff.slice(0, 1000)}`,
            [], prompt, this._settings.model
        );
            ...
            if (data.type === 'message') {
                try {
                    ...
                    if (data.text.toLowerCase().includes('ține minte') || 
                        ...
                        if (memPath) writeMemory(memPath, readMemory(memPath) + entry);
                    }
